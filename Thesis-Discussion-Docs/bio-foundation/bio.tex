...
This chapter is the biological foundation of the cicuit that the model developed in this thesis follows, the neocortical microcircuit 
...
and \textcite{sherman1998, sherman2013} for the thalamus and its cortical connections.
...
The inhibitory interneurons of Section~\ref{sec:bio-inhibitory} are woven into this excitatory template at every stage. In the input layer, thalamocortical afferents contact PV cells as well as excitatory neurons, so that thalamic drive arrives with a brief lag of perisomatic feedforward inhibition that sharpens the timing and gain of the layer-4 response. More generally, because each excitatory projection is accompanied by the compartment-specific inhibition described earlier perisomatic from PV cells, dendritic from SST cells, disinhibitory gating from VIP cells the flow along the spine is everywhere shaped by the balance of excitation and inhibition rather than by excitation alone \parencite{tremblay2016}.
...
\centering
\begin{tikzpicture}[
  lamina/.style={draw, rectangle, minimum width=5cm, minimum height=0.9cm,
                 text width=4.6cm, align=center, font=\small},
  thal/.style={draw, rectangle, rounded corners, minimum width=2.6cm,
               minimum height=1cm, text width=2.4cm, align=center, font=\small},
  trn/.style={draw, rectangle, rounded corners, minimum width=1.6cm,
              minimum height=2.2cm, text width=1.4cm, align=center, font=\small},
  driver/.style={-{Stealth[length=3mm]}, line width=1.1pt},
  modulator/.style={-{Stealth[length=2.5mm]}, line width=0.5pt, gray},
    inhib/.style={-{Bar[width=3mm]}, line width=0.9pt, blue!70!black},
      spine/.style={-{Stealth[length=2mm]}, line width=0.7pt},
        feedback/.style={-{Stealth[length=2.5mm]}, line width=0.8pt, dashed},
]
  % - Cortical column: five laminae, L1 (top) to L6 (bottom) -
  \node[lamina] (L1)  at (0, 8.00) {\textbf{L1}  apical tufts};
  \node[lamina] (L23) at (0, 7.08) {\textbf{L2/3}  associative};
  \node[lamina] (L4)  at (0, 6.16) {\textbf{L4}  granular input};
  \node[lamina] (L5)  at (0, 5.24) {\textbf{L5}  deep output};
  \node[lamina] (L6)  at (0, 4.32) {\textbf{L6}  corticothalamic};

  % - Thalamus: two relays and the TRN gate -
  \node[thal] (FO) at (-1.4, 1.6) {\textbf{First-order relay}\\\footnotesize (LGN, VPM)};
  \node[thal] (HO) at ( 1.7, 1.6) {\textbf{Higher-order relay}\\\footnotesize (pulvinar, Po)};
  \node[trn]  (TRN) at (-4.2, 1.6) {\textbf{TRN}\\\footnotesize gate};

  % - Prethalamic driver source -
  \node[thal, fill=black!5] (PRE) at (-1.4, -0.4) {\textbf{Prethalamic}\\\footnotesize (retina, lemniscus)};
    % - External nodes: feedback source and transthalamic target -
  \node[lamina, minimum width=3cm, text width=2.6cm, fill=black!5]
        (HI) at (6.2, 7.54) {\textbf{Higher cortical area}\\\footnotesize (top-down)};
  \node[thal, fill=black!5]
        (B) at (5.6, 1.6) {\textbf{L4 of area B}\\\footnotesize (transthalamic target)};
  % - DRIVER connections (thick solid) -
  \draw[driver] (PRE) -- (FO);                          % prethalamic -> FO relay
  \draw[driver] (FO.north) to[out=90, in=180] (L4.west);   % FO relay -> L4 (ascending drive)
  \draw[driver] (FO.west) -- (TRN.south east);          % relay collateral -> TRN
  \draw[driver] (L5.east) to[out=0, in=90] (HO.north);  % L5 -> HO relay (column output)
  \draw[driver] (HO.east) -- (B.west);                  % HO relay -> L4 of area B (transthalamic)
    % - MODULATOR connections (thin, descending feedback) -
    \draw[modulator] (L6.south) to[out=270, in=100] (FO.north);       % L6 -> FO relay
  \draw[modulator] (L6.south east) to[out=280, in=100] (HO.north);  % L6 -> HO relay
  \draw[modulator] (L6.west) to[out=180, in=90] (TRN.north);        % L6 -> TRN
    % - INHIBITORY gating (TRN -> FO relay) -
    \draw[inhib] (TRN.south east) -- ([yshift=-3mm]FO.west);   % TRN gates the relay
      % - INTRACOLUMNAR spine (canonical flow, inside the column) -
   % - INTRACOLUMNAR processing (abstracted; sequence given in text) -
    \draw[spine] ([xshift=5mm]L4.west) -- ([xshift=5mm]L6.west)
        node[midway, left=2pt, font=\scriptsize, align=center] {canonical\\flow};
  \draw[spine, densely dotted] ([xshift=-5mm]L6.east) -- ([xshift=-5mm]L4.east)
        node[midway, right=2pt, font=\scriptsize] {\,L6$\to$L4};
      % - TOP-DOWN feedback (dashed; driver/modulator class undetermined) -
  \draw[feedback] (HI.south) to[out=270, in=0] (L1.east);   % higher area -> L1 apical tufts
    % - LEGEND -
  \node[draw, rounded corners, align=left, font=\scriptsize,
        anchor=north west] (leg) at (3.6, 0.3) {%
    \tikz{\draw[driver] (0,0)--(0.7,0);} \ driver (information)\\[2pt]
    \tikz{\draw[modulator] (0,0)--(0.7,0);} \ modulator (gain/state)\\[2pt]
    \tikz{\draw[inhib] (0,0)--(0.7,0);} \ inhibitory (TRN gate)\\[2pt]
    \tikz{\draw[feedback] (0,0)--(0.7,0);} \ top-down feedback (class open)\\[2pt]
    \tikz{\draw[spine, densely dotted] (0,0)--(0.7,0);} \ intracolumnar
  };
\end{tikzpicture}
\caption[The cortico-thalamo-cortical loop]{%
  The cortico-thalamo-cortical loop and the canonical cortical column.
  A first-order relay conveys driving input (thick) from a subcortical
  source to layer~4; the column processes it through its canonical
  intracolumnar flow and returns two descending signals: a modulatory
  layer-6 projection (thin grey) to the relays and the reticular nucleus
  (TRN), which gates the relay (blue), and a driving layer-5 projection
  to a higher-order relay, which drives layer~4 of a second cortical area
  (the transthalamic route). Top-down feedback to the apical tufts in
  layer~1 is drawn dashed, its driver/modulator class left open.
  Connection classes follow \textcite{sherman1998, sherman2013}.}
\label{fig:bio-loop}
\end{figure}