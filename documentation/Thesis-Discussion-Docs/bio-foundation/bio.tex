...
\begin{tikzpicture}[
  ...
  driver/.style={-{Stealth[length=3mm]}, line width=1.3pt},
  modulator/.style={-{Stealth[length=2.5mm]}, line width=0.6pt, gray},
  inhib/.style={-{Bar[width=3mm]}, line width=0.9pt, blue!70!black},
  feedback/.style={-{Stealth[length=2.5mm]}, line width=0.8pt, dashed},
  stepnum/.style={circle, draw, fill=white, inner sep=1pt,
               font=\scriptsize\bfseries, minimum size=5.5mm},
]
  % ---- Cortical column: L1 (top) to L6 (bottom) ----
  ...
  \node[thal, fill=black!5] (PRE) at (-1.4, -0.6) {\textbf{Sensory input}\\\footnotesize (retina, lemniscus)};
  \node[thal, fill=black!5] (B)   at ( 5.6, 1.6) {\textbf{L4 of area B}\\\footnotesize (next area)};
  ...
  % ==== ASCENDING PATH — thick, numbered, going UP (left side) ====
  \draw[driver] (PRE) -- (FO) node[stepnum, midway, right=2mm] {1};
  \draw[driver] (FO.north) to[out=90, in=180] (L4.west) node[stepnum, pos=0.55] {2};
  \draw[driver] ([xshift=-1.6cm]L4.north) -- ([xshift=-1.6cm]L23.south)
        node[stepnum, midway, left=1mm] {3};
  \draw[driver] ([xshift=-1.9cm]L23.north) to[out=90,in=180] ([xshift=-2.2cm]L5.west)
        node[stepnum, pos=0.5, left=1mm] {4};
  \draw[driver] ([xshift=-1.6cm]L5.north) -- ([xshift=-1.6cm]L6.south)
        node[stepnum, midway, left=1mm] {5};

  % ==== DESCENDING OUTPUT — thick driver, L5 -> HO -> next area ====
  \draw[driver] (L5.east) to[out=0, in=90] (HO.north) node[stepnum, pos=0.55] {6};
  \draw[driver] (HO.east) -- (B.west) node[stepnum, midway, above] {7};

  % ==== DESCENDING CONTROL — thin grey modulator (right lane) ====
  \draw[modulator] (L6.south) to[out=270, in=100] (FO.north);
  \draw[modulator] (L6.south east) to[out=280, in=100] (HO.north);
  \draw[modulator] (L6.west) to[out=180, in=90] (TRN.north);

  % ==== TRN gate (inhibitory) ====
  \draw[inhib] (TRN.south east) -- ([yshift=-3mm]FO.west);

  % ==== TOP-DOWN feedback (dashed, separate) ====
  \draw[feedback] (HI.south) to[out=270, in=0] (L1.east);

  % ==== LEGEND ====
  \node[draw, rounded corners, align=left, font=\scriptsize,
        anchor=north west] (leg) at (3.4, 0.4) {%
    \tikz{\draw[driver] (0,0)--(0.7,0);} \ \textbf{1--7} ascending drive \& output (driver)\\[2pt]
    \tikz{\draw[modulator] (0,0)--(0.7,0);} \ L6 modulator (gain/state)\\[2pt]
    \tikz{\draw[inhib] (0,0)--(0.7,0);} \ TRN gate (inhibitory)\\[2pt]
    \tikz{\draw[feedback] (0,0)--(0.7,0);} \ top-down feedback (class open)
  };
\end{tikzpicture}
\caption[The cortico-thalamo-cortical loop]{%
  The cortico-thalamo-cortical loop and the canonical cortical column, traced as
  a numbered flow. \textbf{(1)}~A first-order relay receives driving input from a
  subcortical source and \textbf{(2)}~drives layer~4. \textbf{(3--5)}~The column
  processes it through the canonical intracolumnar flow (L4$\to$L2/3$\to$L5$\to$L6).
  \textbf{(6--7)}~Layer~5 sends a driving projection to a higher-order relay, which
  drives layer~4 of the next cortical area (the transthalamic route). In parallel,
  layer~6 returns a modulatory projection (thin grey) to all relays, the reticular
  nucleus (TRN) gates the relay (blue), and top-down feedback reaches the apical
  tufts in layer~1 (dashed). Connection classes follow \textcite{sherman1998, sherman2013}.}
\label{fig:bio-loop}
\end{figure}